...
2. **Vehicle detection.** The primary detection path is the ESP32-CAM running frame-difference motion detection — it raises a vehicle flag over a UART link to the main board. The hardware also fits four HC-SR04 ultrasonics (two pairs spaced 3 cm apart) intended for direction inference, but in v2.2 only US4 delivers ranging data because of pin-sharing with the 74HC595 and the servos (see [`docs/known_limitations.md`](./docs/known_limitations.md) L8). The FSM ORs the two paths, so vision alone is sufficient to trigger a cycle.
